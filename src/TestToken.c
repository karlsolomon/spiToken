...
/*******************************************************************************
 * @brief testToken_eeprom_writeEnable
 *
 * Verify token can read
 *
 * @param  None
...
{
    int errCount = 0;
    uint8_t tmpReadBuffer[TOKEN_EEPROM_PAGE_LEN] = {0};
    TOKEN_ErrCode_t err = (TOKEN_ErrCode_t) TokenEeprom_Read(TEST_TOKEN_START_ADDR, tmpReadBuffer, TOKEN_EEPROM_PAGE_LEN);
    if(err != TOKEN_ERR_OK)
    {
        errCount++;
        TEST_PRINT_ERROR(err, "");
    }
    TEST_TOKEN_PRINT(m_tokenType, TOKEN_EEPROM, errCount, menu_e_readStr);
    return errCount;
}
...
{
    int errCount = 0;
    TOKEN_ErrCode_t err = TokenEeprom_Erase(TEST_TOKEN_START_ADDR, TOKEN_EEPROM_PAGE_LEN);
    if(!Test_WriteAndVerify(TOK_E_WRITE, TOK_E_READ, TEST_TOKEN_START_ADDR, TOKEN_EEPROM_PAGE_LEN))
    {
        errCount++;
        TEST_PRINT_ERROR(err, "Verify failed");
    }
    TokenEeprom_Erase(TEST_TOKEN_START_ADDR, TOKEN_EEPROM_PAGE_LEN);
    TEST_TOKEN_PRINT(m_tokenType, TOKEN_EEPROM, errCount, menu_e_write1Str);
    return errCount;
}
...
{
    int errCount = 0;
    TOKEN_ErrCode_t err = TokenEeprom_Erase(0, TOKEN_EEPROM_MEM_SIZE);
    if(!Test_WriteAndVerify(TOK_E_WRITE, TOK_E_READ, 0, TOKEN_EEPROM_MEM_SIZE))
    {
        errCount++;
        TEST_PRINT_ERROR(err, "Verify failed");
    }
    TokenEeprom_Erase(TEST_TOKEN_START_ADDR, TOKEN_EEPROM_MEM_SIZE);
    TEST_TOKEN_PRINT(m_tokenType, TOKEN_EEPROM, errCount, menu_e_write2Str);
    return errCount;
}
...
static int testToken_eeprom_eraseTest(void)
{
    ...
    if(!Test_VerifyErased(TOK_E_READ, TEST_TOKEN_START_ADDR, TOKEN_EEPROM_PAGE_LEN))
    {
        errCount++;
        TEST_PRINT_ERROR(err, "Verify failed");
    }
    TEST_TOKEN_PRINT(m_tokenType, TOKEN_EEPROM, errCount, menu_e_erase1Str);
    return errCount;
}


/*******************************************************************************
 * @brief testToken_eeprom_eraseAllTest
 *
 * Verify token can erase all memory through erase cmd
 *
 * @param  None
...
static int testToken_eeprom_eraseAllTest(void)
{
    ...
    if(!Test_VerifyErased(TOK_E_READ, 0, TOKEN_EEPROM_MEM_SIZE))
    {
        errCount++;
        TEST_PRINT_ERROR(err, "Verify failed");
    }
    TEST_TOKEN_PRINT(m_tokenType, TOKEN_EEPROM, errCount, menu_e_erase2Str);
    return errCount;
}

/*******************************************************************************
 * @brief testToken_eeprom_eraseChipTest
 *
 * Verify token can erase chip
 *
 * @param  None
...
{
    int errCount = 0;
    TOKEN_ErrCode_t err = TokenEeprom_EraseAll();
    if(err != TOKEN_ERR_OK)
    {
        errCount++;
        TEST_PRINT_ERROR(err, "");
    ...
    TEST_TOKEN_PRINT(m_tokenType, TOKEN_EEPROM, errCount, menu_e_eraseAllStr);
    return errCount;
}

/*******************************************************************************
 * @brief testToken_eeprom_protectTest
 *
 * Verify token can protect all region combinations
 *
 * @param  None
...
static int testToken_eeprom_protectTest(void)
{
    ...
    for(uint8_t i = 0; i < (uint8_t) TOKEN_EEPROM_PROTECT_COUNT; i++)
    {
        err = TokenEeprom_ProtectRegion((TOKEN_EepromProtect_t) i);
        protectedRegion = (uint8_t) TokenEeprom_GetProtectedRegion();
    ...
    TokenEeprom_ProtectRegion(TOKEN_EEPROM_PROTECT_NONE);

    TEST_TOKEN_PRINT(m_tokenType, TOKEN_EEPROM, errCount, menu_e_protectStr);
    return errCount;
}
